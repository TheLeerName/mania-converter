...
class FlxSlider extends FlxSpriteGroup
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (overlapsPoint(FlxG.mouse.getPositionInCameraView(cameras[0]), true))
		{
			...
			updateValue();
			if (varString == null) {
				if (FlxG.keys.justPressed.LEFT) value -= Math.pow(0.1, decimals);
				else if (FlxG.keys.justPressed.RIGHT) value += Math.pow(0.1, decimals);

				if (FlxG.keys.justReleased.LEFT || FlxG.keys.justReleased.RIGHT) holdTime = 0;

				if (FlxG.keys.pressed.LEFT) {
					holdTime += elapsed;
					if (holdTime >= 0.25) value -= Math.pow(0.1, decimals);
				}
