...
exports.checkOptionsINI = function()
{
	...
	if (file.exists('options.ini'))
	{
		...
		if (ver < latestFormatVersion)
		{
			if (ver < 3)
			{
				optia.splice(utils.findLine(optia, 'EngineSync'), 0, 'IgnoreNote:Hurt Note');
				optia.splice(utils.findLine(optia, 'EngineSync', utils.findLine(optia, 'OPTIONS GUIDE (with default values)')), 0, 'IgnoreNote:Hurt Note |:| for list a ignored notetypes split on comma. (for example, you want convert chart without damage notes)');
			}
	...
	optfile.splice(utils.findLine(optfile, 'EngineSync'), 0, 'IgnoreNote:Hurt Note');
	optfile.splice(utils.findLine(optfile, 'EngineSync', utils.findLine(optfile, 'OPTIONS GUIDE (with default values)')), 0, 'IgnoreNote:Hurt Note |:| for list a ignored notetypes split on comma. (for example, you want convert chart without damage notes)');
