...
function checkFolders(path)
{
	...
	{
		stri += ar[i] + '/';
		if (!fs.existsSync(stri.substring(0, stri.length - 1)))
			fs.mkdirSync(stri.substring(0, stri.length - 1));
	}
	return true;
}
exports.parseTXT = function(path, fromFile = true)
{
	...
	if (fromFile)
	{
		...
		if (exists(path))
			...
			console.log('Can\'t parse ' + path + ', file is not exist!');
	}
	else
		daList = path.split('\n');
...
exports.stringify = function(path, string = "\t", fromFile = true)
{
	...
	if (fs.existsSync(path) && fromFile)
		...
		console.log('Can\'t stringify ' + path + ', file is not exist!');
}	
...
exports.readDir = function(path)
{
	...
	if (fs.lstatSync(path).isDirectory())
	{
		...
		console.log('Can\'t read directory ' + path + ', directory is not exist!');
}
...
function createDir(path)
{
	...
	if (!fs.existsSync(path))
		...
		console.log('Can\'t create directory ' + path + ', directory already exist!');
}
...
exports.deleteFile = function(path)
{
	...
	//else
		//console.log('Can\'t delete file ' + path + ', file is not exist!');
}
...
exports.getContent = function(path)
{
	...
	if (fs.existsSync(path))
		...
		console.log('Error: Can\'t get content from ' + path + ', file is not exist!');
}
